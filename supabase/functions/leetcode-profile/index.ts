...
serve(async (req) => {
  ...
  try {
    ...
    const solvedByDifficulty = (user.submitStatsGlobal?.acSubmissionNum || []) as DifficultyCount[];
    const easySolved = solvedByDifficulty.find((s) => s.difficulty === 'Easy')?.count || 0;
    const mediumSolved = solvedByDifficulty.find((s) => s.difficulty === 'Medium')?.count || 0;
    const hardSolved = solvedByDifficulty.find((s) => s.difficulty === 'Hard')?.count || 0;
    const totalSolved = easySolved + mediumSolved + hardSolved;

    // Parse total questions
    const easyTotal = (allQuestions as DifficultyCount[]).find((q) => q.difficulty === 'Easy')?.count || 0;
    const mediumTotal = (allQuestions as DifficultyCount[]).find((q) => q.difficulty === 'Medium')?.count || 0;
    const hardTotal = (allQuestions as DifficultyCount[]).find((q) => q.difficulty === 'Hard')?.count || 0;

    // Parse languages
    const languages = ((user.languageProblemCount || []) as LanguageCount[])
